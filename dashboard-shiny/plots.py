...
def line_plot_videos_views(df, selected_var):
    ...
    fig = px.bar(
        videos_count,
        x=videos_count.index,
        y=videos_count.values,
        labels={"y": "Count"},
        title="Videos Count and Average Views per Year",
    )
    fig.add_scatter(
        x=avg_views.index,
        y=avg_views.values,
...
def sunburst_df(df):
    ...
    hierarchy_df = pd.merge(
        hierarchy_df, data_count, how="left", left_on="Child", right_on="category"
    )
...
def category_popularity_over_time(df, selected_category=None):
    ...
    if selected_category and selected_category != "All categories":
        fig.update_traces(
            selector=dict(name=selected_category), line=dict(width=2, color="black")
        )
...
def sentiment_analysis_plot(df, selected_var):
    ...
    sentiment_percentages_melted = sentiment_percentages.melt(
        id_vars="year", var_name="sentiment", value_name="percentage"
    )
...
def log_scale_boxplot_by_categories(
    df, start_date, end_date, selected_categories=None, selected_lang="All Languages"
):
    """
    ...
    fig = px.box(
        ...
        x="parent",
        y="views",
        title="Log-Scale Views by Selected Parent Categories",
    )
...
def views_vs_title_length_plot(
    df, start_date, end_date, selected_categories=None, selected_lang="All Languages"
):
    """
